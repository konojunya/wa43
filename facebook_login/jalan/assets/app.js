...
App = {
  ...
  render: function () {
    ...
    $plans.each(function () {
      ...
      view += '<img src="' + $plan.find('PlanPictureURL').text() + '" class="responsive-image" />';
      ...
      view += '<ul>';
      view += '<li class="left"><i class="material-icons left">business</i>' + $plan.find('HotelName').text() + '</li>';
      view += '<li class="left"><i class="material-icons left">payment</i>' + $plan.find('SampleRate').text() + '円〜</li>';
      view += '</ul>';

      view += '<p class="clearfix">' + $plan.find('PlanName').text() + '</p>';

      view += '<a target="_blank" href="' + $plan.find('PlanDetailURL').text() + '" class="btn btn-block blue">プランの詳細を見る</a>';
